...
Here in John's coordinate system with basis vectors $b_{1}=\begin{bmatrix}1\\0\end{bmatrix}$ and $b_{2}=\begin{bmatrix}0\\1\end{bmatrix}$ a vector exist which in his perspective is at $\begin{bmatrix}-1\\2\end{bmatrix}$ , hence vector is formed by linear combination of $v_{B}=-1b_{1}+2b_{2}$, i.e., go 1 unit in opposite direction of $b_{1}$ and go 2 unit in direction of $b_{2}$. when we come to our world we notice that John's basis vectors in our coordinate system is at coordinate $\vec{b_{1}}=\begin{bmatrix}2\\1\end{bmatrix}$ and $\vec{b_{2}}=\begin{bmatrix}-1\\1\end{bmatrix}$ we already know Linear combination does not change ( go 1 unit in opposite direction of $b_{1}$ and go 2 unit in direction of $b_{2}$) hence we can multiply and add to get the coordinate of vector in our coordinate system i.e., $\begin{bmatrix}-4\\1\end{bmatrix}$ .
Here we can view the change in basis as linear transformation :
...
If a vector is in our coordinate and if we want to see its coordinate in John's world we simply take inverse matrix of John's basis vectors in our coordinate system.
